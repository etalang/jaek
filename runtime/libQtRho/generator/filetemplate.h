...
struct FileTemplate
{
    FileTemplate(string idlName, string inFileName, string outFileName):
        idlName(idlName), inFileName(inFileName), outFileName(outFileName)
    {
        hasIn = true;
        isOK  = true;
        lines = 0;
        ...
        if (in.fail()) {
            std::cerr << "Warning: no input template" << inFileName << "\n";
            hasIn = false;
        }

        if (isOK) {
            out << "// WARNING: Portions of this file are autogenerated from " << idlName << ".idl and " << inFileName << ".\n";
