...
sealed class CFGNode(
    ...
    class Gets(val varName: String, val expr: CFGExpr, edge: Edge.Lazy) : CFGNode(edge) {
        override val pretty = "$varName ← ${expr.pretty}"
    }

    class Mem(val loc: CFGExpr, val expr: CFGExpr, edge: Edge.Lazy) : CFGNode(edge) {
    ...
    class Funcking(val name: String, val movIntos: List<String>, val args: List<CFGExpr>, edge: Edge.Lazy) :
        CFGNode(edge) {
