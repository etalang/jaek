...
Lexer::Lexer(istream* _stream): stream(_stream), charLoaded(false), lineNum(0)
{
    ...
    keywords["pointer"] = Pointer;
    keywords["nopeer"]  = NoPeer;
    keywords["xi"]      = Xi;
    keywords["bindH"]   = BindH;
    keywords["bindCPP"] = BindCpp;
    keywords["enumConst"]  = EnumConst;
    keywords["enumPrefix"] = EnumPrefix;
    lineNum = 1;
}
...
Lexer::Token Lexer::lexComment()
{
    ...
    if (in == '/') {
        ...
        text.resize(text.size() - 1); // strip the trailing *
        getNext(); // Eat the /
    } else {
