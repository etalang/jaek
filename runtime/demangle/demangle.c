/**
 A simple demangling filter for Eta. (Similar to c++filt). Very basic,
...
/**
 Tries to demangle Eta initialization/info symbols.
*/
char* tryDemangleEtaSetup(char** posPtr) {
...
/*
 Tries to demangle Eta global symbols.
*/
char* tryDemangleEtaGlobal(char** posPtr) {
    // _I_g_<name>_<type>..
    char* pos = *posPtr;
    char* cfy, *name, *type;
...
/**
 Checks whether `test' starts with the string `start'.
 Returns -1 if it does not, and the position after the start string if it does
 */
int startsWith(const char* test, const char* start) {
  int pos = 0;
...
char* tryDemangleSymbol(char** posPtr) {
    ...
    int allocStart = startsWith(pos, "_eta_alloc");
    if (allocStart >= 0) {
      *posPtr = pos + allocStart;
      decl = strdup("[etaruntime]alloc(int): ?[]");
      return decl;
    }

    int OOBStart = startsWith(pos, "_eta_out_of_bounds");
    if (OOBStart >= 0) {
      *posPtr = pos + OOBStart;
      decl = strdup("[etaruntime]out_of_bounds()");
      return decl;
    }
    ...
    // here, we may have _ in case of globals
    if (pos[0] == '_') {
        char* ixInfo;

        ++pos;
        ixInfo = tryDemangleEtaSetup(&pos);
        if (ixInfo) {
            *posPtr = pos;
            return ixInfo;
        }

        ixInfo = tryDemangleEtaGlobal(&pos);
        if (ixInfo) {
            *posPtr = pos;
            return ixInfo;
        }
