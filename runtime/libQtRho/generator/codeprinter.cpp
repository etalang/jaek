...
ostream& CodePrinter::ind(CodeStream strNum, int ind)
{
    ostream& str = this->operator()(strNum);
    for (int i = 0; i < ind; ++i)
        str << ' ';
    return str;
}
...
void CodePrinter::printCode(ostream& out, int baseIndent, const string& code,
