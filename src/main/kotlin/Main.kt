...
class Etac(val disableOutput: Boolean = false) : CliktCommand(printHelpOnEmptyArgs = true) {
    ...
    private val disableOpt: Boolean by option(
        "-O", help = "Prevents optimizations (e.g. constant folding) from happening."
    ).flag()
    ...
    private val initOutputIR: Boolean by option(
        "--irgen", help = "Generate intermediate representation as SExpr"
    ).flag()
    private val runIR: Boolean by option(
        "--irrun", help = "Generate and interpret intermediate representation (not fully supported)"
    ).flag()
    ...
    override fun run() {
        ...
        folderFiles.forEach {
            ...
            if (it.exists() && (it.extension == "eta" || it.extension == "eti" || it.extension == "rh" || it.extension == "ri")) {
                ...
                val optIRInitialFile = if (printIROpts.contains("initial")) getOutFileName(
                    it, absDiagnosticPath, "_initial.ir"
                ) else null
                val optIRFinalFile =
                    if (printIROpts.contains("final")) getOutFileName(it, absDiagnosticPath, "_final.ir") else null
                val optCFGInitialFile: File? = if (printCFGOpts.contains("initial")) getOutFileName(
                    it, absDiagnosticPath, ".ignored"
                ) else null
                            ...
                            when (ast) {
                                is Program -> {
                                    ...
                                    try {
                                        ...
                                        val assemblyAssembler = AssemblyGenerator(ir, funcMap,optConfig.desire(Opt.Actions.reg))
                                        // print to file.s
